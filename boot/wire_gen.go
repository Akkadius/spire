...
func InitializeApplication() (App, error) {
	...
	questExamplesGithubSourcer := questapi.NewExamplesGithubSourcer(logger, cache, githubSourceDownloader)
	questApiController := questapi.NewController(logger, parseService, questExamplesGithubSourcer)
	...
	clientFilesController := clientfiles.NewController(logger, exporter, importer, databaseResolver)
	staticMapController := staticmaps.NewStaticMapController(databaseResolver, logger)
	releases := eqemuanalytics.NewReleases()
	eqemuanalyticsAnalyticsController := eqemuanalytics.NewController(logger, databaseResolver, releases)
	authedAnalyticsController := eqemuanalytics.NewAuthedController(logger, databaseResolver)
	changelog := eqemuchangelog.NewChangelog()
	eqemuChangelogController := eqemuchangelog.NewController(logger, databaseResolver, changelog)
	...
	v := ProvideCommands(helloWorldCommand, adminPingOcculus, userCreateCommand, generateModelsCommand, generateControllersCommand, httpServeCommand, routesListCommand, generateConfigurationCommand, migrateCommand, parseCommand, exampleTestCommand, generateRaceModelMapsCommand, changelogCommand, testFilesystemCommand, initCommand, userChangePasswordCommand, occulusUpdateCommand, serverLauncherCommand, crashAnalyticsFingerprintBackfillCommand, updateCommand, scrapeCommand, importCommand)
