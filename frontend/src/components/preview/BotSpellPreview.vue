<template>
  <div v-if="spells">
    <div
      ...
      v-if="spells && !spells.bot_spells_entries"
    >
    ...
    <table
      ...
      v-if="spells && spells.bot_spells_entries"
      id="bot-spell-preview-list-table"
    >
      ...
      >
        <td
          ...
          v-if="editButtons"
        >
          {{(e.is_parented ? `Parent List (${e.bot_spells_id})` : '')}}
        ...
        <td>{{ (BOT_SPELL_TYPES[e.type] ? BOT_SPELL_TYPES[e.type] : "") }}</td>
        <td>{{ getManacost(e) }}</td>
        <td>{{ e.priority }}</td>
        ...
        <td>{{ e.maxlevel }}</td>
        <td>{{ e.bucket_name }}</td>
        <td>{{ e.bucket_value }}</td>
        <td>{{ e.bucket_comparison }}</td>
      </tr>
      </tbody>
    </table>
...
export default {
  name: "BotSpellPreview",
  components: { EqDebug, SpellPopover },
  data() {
    return {
      BOT_SPELL_TYPES: BOT_SPELL_TYPES,
  ...
  methods: {
    ...
    async deleteSpellListEntry(e) {
      if (confirm(`Are you sure you want to delete this spell? \n\n(${e.spells_new.name}) (${e.spells_new.id})?`)) {
        ...
        try {
          await (new BotSpellsEntryApi(...SpireApi.cfg()))
            .deleteBotSpellsEntry({ id: e.id })
            .then(() => {
    ...
    async init() {
      ...
      let spellsList = this.spells && this.spells.bot_spells_entries && this.spells.bot_spells_entries.length > 0
        ? JSON.parse(JSON.stringify(this.spells.bot_spells_entries))
        : []

      if (this.spells.parent_list > 0) {
        try {
          const BotSpellsClient = (new NpcSpellApi(...SpireApi.cfg()))
          const r               = await BotSpellsClient.getNpcSpell({ id: this.spells.parent_list },
            {
              query:
          ...
          for (const [i, e] of r.data.bot_spells_entries.entries()) {
            r.data.bot_spells_entries[i].is_parented = true
          }

          if (r.status === 200 && r.data && r.data.bot_spells_entries) {
            spellsList = spellsList.concat(r.data.bot_spells_entries)
          }
        } catch (err) {
          if (err.response.data.error) {
      ...
      // table sort
      const target = document.getElementById('bot-spell-preview-list-table')
      if (target) {
        setTimeout(() => {
