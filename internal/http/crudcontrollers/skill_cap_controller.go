...
func (e *SkillCapController) createSkillCap(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Created [SkillCap] [%v] data [%v]", skillCap.SkillID, strings.Join(fields, ", "))
...
func (e *SkillCapController) deleteSkillCap(c echo.Context) error {
	...
	if e.db.GetSpireDb() != nil {
		...
		// record event
		event := fmt.Sprintf("Deleted [SkillCap] [%v] keys [%v]", result.SkillID, strings.Join(ids, ", "))
