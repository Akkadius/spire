...
func (e *HackerController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodGet, "hackers/count", e.getHackersCount, nil),
...
// @Success 200 {array} models.Hacker
// @Failure 500 {string} string "Bad query request"
// @Router /hackers/count [get]
func (e *HackerController) getHackersCount(c echo.Context) error {
	var count int64
	err := e.db.QueryContext(models.Hacker{}, c).Count(&count).Error
	if err != nil {
		return c.JSON(http.StatusInternalServerError, echo.Map{"error": err})
	}
