...
func (e *CharacterPetInventoryController) updateCharacterPetInventory(c echo.Context) error {
	...
	err = e.db.QueryContext(models.CharacterPetInventory{}, c).Select("*").Session(&gorm.Session{FullSaveAssociations: true}).Updates(&request).Error
	if err != nil {
...
func (e *CharacterPetInventoryController) createCharacterPetInventory(c echo.Context) error {
	...
	err := e.db.Get(models.CharacterPetInventory{}, c).Model(&models.CharacterPetInventory{}).Create(&characterPetInventory).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
