...
func (e *CharacterAltCurrencyController) updateCharacterAltCurrency(c echo.Context) error {
	...
	err = e.db.QueryContext(models.CharacterAltCurrency{}, c).Select("*").Session(&gorm.Session{FullSaveAssociations: true}).Updates(&request).Error
	if err != nil {
...
func (e *CharacterAltCurrencyController) createCharacterAltCurrency(c echo.Context) error {
	...
	err := e.db.Get(models.CharacterAltCurrency{}, c).Model(&models.CharacterAltCurrency{}).Create(&characterAltCurrency).Error
	if err != nil {
		return c.JSON(
			http.StatusInternalServerError,
