...
func (e *NpcTypeController) updateNpcType(c echo.Context) error {
    ...
    entity := models.NpcType{}
	err := e.db.Get(models.NpcType{}, c).Model(&models.NpcType{}).First(&entity, npcType.ID).Error
	if err != nil || npcType.ID == 0 {
		return c.JSON(http.StatusNotFound, echo.Map{"error": "Cannot find entity"})
	}

	err = e.db.Get(models.NpcType{}, c).Model(&entity).Select("*").Updates(&npcType).Error
	if err != nil {
