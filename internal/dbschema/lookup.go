...
func NewLookup(db *sql.DB, logger *logger.AppLogger) *Lookup {
	return &Lookup{db: db, logger: logger}
}
...
func (c *Lookup) GetSchemas() (map[string][]DbSchemaRowResult, error) {
	if schemas != nil {
		return schemas, nil
	}
...
func (c *Lookup) GetTableNames() ([]string, error) {
...
func (c *Lookup) GetSchemasByTableName(tableName string) ([]DbSchemaRowResult, error) {
	schemas, err := c.GetSchemas()
	if err != nil {
		return nil, err
	}
