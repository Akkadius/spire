...
func (e *LoginAccountController) Routes() []*routes.Route {
	return []*routes.Route{
		...
		routes.RegisterRoute(http.MethodPost, "spells_news/bulk", e.getLoginAccountsBulk, nil),
...
func (e *LoginAccountController) updateLoginAccount(c echo.Context) error {
	...
	err := e.db.Get(models.LoginAccount{}, c).Model(&models.LoginAccount{}).First(&models.LoginAccount{}, loginAccount.ID).Error
	if err != nil || loginAccount.ID == 0 {
		return c.JSON(http.StatusNotFound, echo.Map{"error": "Cannot find entity"})
	}

	err = e.db.Get(models.LoginAccount{}, c).Model(&models.LoginAccount{}).Updates(&loginAccount).Error
	if err != nil {
