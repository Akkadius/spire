...
var addr = flag.String("addr", ":3001", "TCP address to listen on")
...
func imageProxyHandler() {
	...
	err := cache.Set("memory:1000:200000h")
	//err := cache.Set(fmt.Sprintf("file:%v/imageproxy", os.TempDir()))
	if err != nil {
		log.Fatal(err)
	}
	...
	fmt.Printf("⇨ [imageproxy] listening on [%s] at [/*]\n", server.Addr)
